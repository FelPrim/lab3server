#!/usr/bin/env python3
"""
Обновленный тестовый клиент для проверки исправлений
"""
...
class TestClient:
    ...
    def _udp_handshake_loop(self):
        ...
        handshake_count = 0
        while not self.udp_handshake_complete and handshake_count < 100:  # Максимум 100 попыток
        ...
        if not self.udp_handshake_complete:
            logger.warning("UDP handshake failed after 100 attempts")
    
    def _send_udp_handshake(self):
        """Отправка UDP handshake пакета"""
        try:
            packet = struct.pack('!Q I', 0, self.connection_id)  # 8 нулей + connection_id
            self.udp_socket.sendto(packet, (self.server_host, self.udp_port))
        except Exception as e:
            logger.error(f"UDP handshake send error: {e}")
    ...
    def _process_tcp_message(self, data):
        ...
        logger.info(f"Received TCP message: type=0x{message_type:02x}, len={len(payload)}")
    ...
    def _process_udp_packet(self, data, addr):
        ...
        logger.info(f"Received UDP packet from {addr}, len={len(data)}")
        ...
        if len(data) >= 12:
            ...
            if first_8_bytes == b'\x00\x00\x00\x00\x00\x00\x00\x00':
                # Это handshake пакет - игнорируем, так как мы клиент
                pass
            else:
                # Это stream packet
                ...
                logger.info(f"Received UDP stream packet: call={call_id}, stream={stream_id}, packet={packet_num}")
    
    def _handle_handshake_start(self, payload):
        """Обработка SERVER_HANDSHAKE_START"""
        if len(payload) >= 4:
            self.connection_id = struct.unpack('!I', payload[:4])[0]
            logger.info(f"Handshake started, connection_id: {self.connection_id}")
    ...
    def _handle_call_joined(self, payload):
        ...
        if len(payload) >= 6:
            ...
            logger.info(f"Call joined: {call_id}, participants={participant_count}, streams={stream_count}")
    ...
    def _handle_call_conn_new(self, payload):
        ...
        if len(payload) >= 8:
            ...
            logger.info(f"New participant in call {call_id}: {connection_id}")
    ...
    def _handle_call_conn_left(self, payload):
        ...
        if len(payload) >= 8:
            call_id = struct.unpack('!I', payload[:4])[0]
            connection_id = struct.unpack('!I', payload[4:8])[0]
            logger.info(f"Participant left call {call_id}: {connection_id}")
    ...
    def _handle_call_stream_new(self, payload):
        ...
        if len(payload) >= 8:
            ...
            logger.info(f"New stream in call {call_id}: {stream_id}")
    ...
    def _handle_call_stream_deleted(self, payload):
        ...
        if len(payload) >= 8:
            call_id = struct.unpack('!I', payload[:4])[0]
            stream_id = struct.unpack('!I', payload[4:8])[0]
            logger.info(f"Stream deleted from call {call_id}: {stream_id}")
    ...
    def send_tcp_message(self, message_type, payload=b''):
        ...
        try:
            ...
            logger.info(f"Sent TCP message: type=0x{message_type:02x}, len={len(payload)}")
            return True
        except Exception as e:
    ...
    def send_udp_stream_packet(self, call_id, stream_id, packet_num, data):
        ...
        try:
            ...
            # Ограничиваем размер данных до 1188 байт
            packet_data = data[:1188] if len(data) > 1188 else data
            packet = header + packet_data
            # Дополняем пакет до 1200 байт если нужно
            if len(packet) < 1200:
                packet += b'\x00' * (1200 - len(packet))
            
            self.udp_socket.sendto(packet, (self.server_host, self.udp_port))
            logger.info(f"Sent UDP packet: call={call_id}, stream={stream_id}, packet={packet_num}")
            return True
        except Exception as e:
            logger.error(f"UDP stream send error: {e}")
            return False
    ...
    def leave_call(self, call_id):
        ...
        if self.send_tcp_message(MessageType.CLIENT_CALL_CONN_LEAVE.value, payload):
            ...
            if success:
                logger.info(f"Left call successfully: {call_id}")
    ...
    def create_stream(self, call_id=0):
        """Создание стрима"""
        logger.info(f"Creating stream for call: {call_id}")
        payload = struct.pack('!I', call_id)
    ...
    def join_stream(self, stream_id):
        ...
        if self.send_tcp_message(MessageType.CLIENT_STREAM_CONN_JOIN.value, payload):
            ...
            if response:
                logger.info(f"Joined stream successfully: {stream_id}")
                return True
        return False
    
    def leave_stream(self, stream_id):
        """Выход из стрима"""
        logger.info(f"Leaving stream: {stream_id}")
        payload = struct.pack('!I', stream_id)
        if self.send_tcp_message(MessageType.CLIENT_STREAM_CONN_LEAVE.value, payload):
            success = self.wait_for_message(MessageType.SERVER_SUCCESS.value)
            if success:
                logger.info(f"Left stream successfully: {stream_id}")
                return True
        return False
    
    def send_test_udp_packets(self, call_id, stream_id, count=10):
        """Отправка тестовых UDP пакетов"""
        logger.info(f"Sending {count} test UDP packets for call={call_id}, stream={stream_id}")
    ...
    def test_join_own_call(self, call_id):
        ...
        logger.info(f"Testing join to own call: {call_id}")
        result = self.join_call(call_id)
        # Ожидаем False, так как нельзя присоединиться к звонку, в котором уже находишься
        if not result:
            logger.info("✓ Correctly rejected join to own call")
            return True
        else:
            logger.error("✗ Unexpectedly allowed join to own call")
            return False
        ...
        if self.send_tcp_message(MessageType.CLIENT_CALL_CONN_JOIN.value, payload):
            if expect_success:
                ...
                if error:
                    ...
                    return True  # Ожидаемая ошибка - тест пройден
...
def run_improved_multiple_clients_test():
    """Улучшенный тест с несколькими клиентами"""
    logger.info("=== Improved multiple clients test ===")
    
    clients = []
    try:
        # Создаем 3 клиента с ID
        for i in range(3):
        ...
        if len(clients) < 3:
            logger.error("Need 3 clients for this test")
            return False
        ...
        time.sleep(1)
        logger.info("=== Improved multiple clients test PASSED ===")
        return True
        
    except Exception as e:
        logger.error(f"Improved multiple clients test failed: {e}")
        return False
    finally:
...
def run_basic_test():
    ...
    try:
        ...
        # Проверяем, что пакеты дошли (через UDP, а не TCP)
        time.sleep(1)
        ...
        pub_stream_id = client.create_stream(0)  # call_id=0 для публичного стрима
...
def run_multiple_clients_test():
    ...
    try:
        ...
        if len(clients) < 2:
            logger.error("Need at least 2 clients for this test")
            return False
        ...
        # Клиент 2 присоединяется к звонку
        if not clients[1].join_call(call_id):
            logger.error("Client 2 failed to join call")
            return False
        
        # Клиент 1 создает стрим
        stream_id = clients[0].create_stream(call_id)
        if not stream_id:
            logger.error("Failed to create stream")
            return False
        
        # Клиент 2 присоединяется к стриму
        if not clients[1].join_stream(stream_id):
            logger.error("Client 2 failed to join stream")
            return False
        
        # Клиент 1 отправляет UDP пакеты
        clients[0].send_test_udp_packets(call_id, stream_id, 3)
        
        # Проверяем, что клиент 2 получает UDP пакеты
        time.sleep(2)
        udp_packets = clients[1].wait_for_udp_packets(count=3, timeout=5)
        if udp_packets and len(udp_packets) >= 2:
            logger.info(f"Client 2 received {len(udp_packets)} UDP packets")
        else:
            logger.warning("Client 2 received fewer UDP packets than expected")
        
        # Клиент 3 пытается присоединиться к несуществующему звонку (тест ошибки)
        fake_call_id = 123456789
        if not clients[2].join_call(fake_call_id):
            logger.info("✓ Correctly rejected join to non-existent call")
        else:
            logger.error("✗ Unexpectedly allowed join to non-existent call")
            return False
...
if __name__ == "__main__":
    print("Video Conference Server Test Client")
    print("====================================")
    ...
    # Даем серверу время на восстановление
    ...
    if run_improved_multiple_clients_test():
        print("✓ Improved multiple clients test PASSED")
    else:
        print("✗ Improved multiple clients test FAILED")
        exit(1)
